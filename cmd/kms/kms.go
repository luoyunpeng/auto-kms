...
type KmsAudio struct {
	stream       beep.StreamSeekCloser
	kmsAudioInfo *beep.Format
	playButton   *widget.Button

	lights [16]time.Duration
	lenMap map[int]time.Duration
}

func main() {
	a := app.NewWithID("ke mu san")
	a.SetIcon(theme.FyneLogo())
	w := a.NewWindow("科目三")
	kmsLabel := widget.NewLabelWithStyle("night light", 1, fyne.TextStyle{Bold: true})

	var kmsAudio KmsAudio
	initLightsAndLen(&kmsAudio)
	InitPlayButton(&kmsAudio)
	...
	kmsAudio.stream = streamer
	kmsAudio.kmsAudioInfo = &format
...
func InitPlayButton(kms *KmsAudio) {
	...
	kms.playButton = playButton
}

func play(kms *KmsAudio) {
	//stream.Seek(kmsAudioInfo.SampleRate.N(lights[0]))
	audios := randomAudioIndex(1, 14)
	log.Println("random:", audios)

	for _, ai := range audios {
		kms.stream.Seek(kms.kmsAudioInfo.SampleRate.N(kms.lights[ai]))
		log.Println("play", ai)
		go P(kms)
		time.Sleep(kms.lenMap[ai])
		if ai != 15 {
			speaker.Clear()
...
func P(kms *KmsAudio) {
	speaker.Play(beep.Seq(kms.stream, beep.Callback(func() {
		kms.playButton.Enable()
	})))
}

// random audio
func randomAudioIndex(start int, end int) []int {
	// range check
...
func initLightsAndLen(kms *KmsAudio)  {
	kms.lights = [16]time.Duration{
	...
	kms.lenMap = map[int]time.Duration{
		0:  time.Second * 4,
		1:  time.Second * 6,
		2:  time.Second * 3,
